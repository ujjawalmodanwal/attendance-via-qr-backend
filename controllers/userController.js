...
const getUserAttendance = async(req, res) => {
	if(req.user.isadmin){
		const getAllUserAttendace = await pool.query(`SELECT users.userid, users.name, users.department, users.class, users.year, CAST(( (attendance.in_time AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Kolkata') AS TIME) AS in_time, CAST(( (attendance.in_time AT TIME ZONE 'UTC') AT TIME ZONE 'Asia/Kolkata') AS TIME) AS out_time FROM attendance LEFT JOIN users ON attendance.userid=users.userid WHERE Date(in_time)='${req.query.requestedDate}';`)
